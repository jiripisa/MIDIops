...
inline void drawBerlinMultiRoll(Display& d, const BerlinRollVoice* vs, int n) {
    ...
    for (int v = 0; v < n; ++v) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            t = t * sat / 255;
            // Accents are NOT drawn white here (unlike the old single-voice roll):
            // white would destroy the voice-identity color coding. Velocity brightness
            // already carries the dynamics — louder accents appear brighter within
            // each voice's color.
